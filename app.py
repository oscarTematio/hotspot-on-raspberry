...
def formulaire():
    ...
    if request.method == 'POST': 
        ...
        c_name = liste[0]
        c_pass = liste[2]
        status = os.system('sudo systemctl status hostapd.service')
        
        if request.form.get('check') == 'on' and status == 768:
            os.system('sudo systemctl start hostapd.service')
            return render_template('started.html')

        elif request.form.get('check') == 'on' and status == 0:
            error = "the hostpot is already started with the ssid {}".format(c_name)
